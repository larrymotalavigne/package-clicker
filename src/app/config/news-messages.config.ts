...
const DAY_OF_WEEK_MESSAGES: NewsMessage[] = [
  {
    text: 'Ugh, Monday. Even the packages look tired.',
    condition: () => new Date().getDay() === 1,
  },
  {
    text: 'Monday motivation: You are only 1,000 clicks away from greatness.',
    condition: () => new Date().getDay() === 1,
  },
  {
    text: 'Taco Tuesday at the sorting facility. Morale is up 200%.',
    condition: () => new Date().getDay() === 2,
  },
  {
    text: 'Tuesday tip: Upgrades are more cost-effective than you think.',
    condition: () => new Date().getDay() === 2,
  },
  {
    text: 'Hump day! Halfway to the weekend. Packages are optimistic.',
    condition: () => new Date().getDay() === 3,
  },
  {
    text: 'Wednesday wisdom: Buy upgrades before buildings.',
    condition: () => new Date().getDay() === 3,
  },
  {
    text: 'Almost Friday! The packages can feel it.',
    condition: () => new Date().getDay() === 4,
  },
  {
    text: 'Thursday throwback: Remember your first click?',
    condition: () => new Date().getDay() === 4,
  },
  {
    text: 'TGIF! The packages are ready to party.',
    condition: () => new Date().getDay() === 5,
  },
  {
    text: 'Friday feeling: Golden packages seem extra shiny today.',
    condition: () => new Date().getDay() === 5,
  },
  {
    text: 'Weekend shift! Double overtime for all facilities.',
    condition: () => new Date().getDay() === 6,
  },
  {
    text: 'Saturday chill: Take it easy. Or click harder. Your choice.',
    condition: () => new Date().getDay() === 6,
  },
  {
    text: 'Sunday rest... just kidding. Packages never sleep.',
    condition: () => new Date().getDay() === 0,
  },
  {
    text: 'Sunday special: Wrinklers seem extra hungry today.',
    condition: () => new Date().getDay() === 0,
  },
];
