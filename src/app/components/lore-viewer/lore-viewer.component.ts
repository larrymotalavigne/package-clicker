    ...
    <div class="lore-overlay" (click)="closePanel.emit()">
      <div class="lore-panel" (click)="$event.stopPropagation()">
        <div class="panel-header">
          <h2>📜 Story</h2>
          <span class="lore-count"
            >{{ entries.length }} / {{ total }} chapters</span
          >
        ...
        <div class="lore-list">
          @for (entry of entries; track entry.id) {
            <div
              class="lore-entry"
              (click)="selectedId = selectedId === entry.id ? '' : entry.id"
            >
          ...
          @if (entries.length === 0) {
            <div class="no-lore">
              No chapters unlocked yet. Keep playing to discover the story!
            </div>
          }
        </div>
      </div>
