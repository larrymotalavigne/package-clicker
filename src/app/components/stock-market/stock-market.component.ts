...
@Component({
  ...
  styles: [
    `
      ...
      .bar-up {
        background: rgba(124, 197, 118, 0.6);
      }
      .bar-down {
...
export class StockMarketComponent {
  ...
  @Output() buyStock = new EventEmitter<{
    buildingId: string;
    quantity: number;
  }>();
  @Output() sellStock = new EventEmitter<{
    buildingId: string;
    quantity: number;
  }>();
