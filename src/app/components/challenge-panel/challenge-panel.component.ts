      ...
      <div class="challenge-panel" (click)="$event.stopPropagation()">
        ...
        @if (active) {
          <div class="active-challenge">
            ...
            <div class="progress-bar">
              <div
                class="progress-fill"
                [style.width.%]="(active.progress / active.target) * 100"
              ></div>
            </div>
            ...
            <div class="time-remaining">
              {{ formatMs(active.remainingMs) }} remaining
            </div>
          </div>
        }
        ...
        @if (result) {
          ...
          >
            {{
              result.success
                ? 'Challenge Complete! +' + result.reward + ' EP'
                : 'Challenge Failed!'
            }}
          </div>
        }
          ...
          @for (ch of available; track ch.id) {
            <div class="challenge-card" [class.disabled]="!!active">
              ...
              <div class="ch-info">
                ...
                <div class="ch-reward">
                  Reward: {{ ch.reward.expressPoints }} EP
                </div>
              </div>
              <button
                class="start-btn"
                [disabled]="!!active"
                (click)="startChallenge.emit(ch.id)"
              >
                Start
              </button>
            </div>
